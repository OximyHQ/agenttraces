...
export const MCP_TOOLS: Tool[] = [
  ...
  { name: "prepare_trace_enrichment", description: "Return a bounded trace excerpt for the current subscribed agent to title and summarize. This never calls a managed model.", inputSchema: objectSchema({ trace_id: string }, ["trace_id"]) },
  { name: "save_trace_enrichment", description: "Cache a title, summary, stages, and outcome generated by the current subscribed agent.", inputSchema: objectSchema({ trace_id: string, title: string, summary: string, stages: { type: "array", items: objectSchema({ kind: { type: "string", enum: ["understand", "build", "edit", "verify", "cleanup", "outcome"] }, text: string }, ["kind", "text"]) }, outcome: { type: "string", enum: ["completed", "partial", "abandoned", "failed", "unknown"] }, model: string, prompt_version: string }, ["trace_id", "title", "summary", "stages", "outcome", "prompt_version"]) },
  ...
  call(name: string, input: Input = {}) {
    switch (name) {
      ...
      case "get_pr_trace": {
        ...
        return this.store.getPullRequestTrace(repository, pullRequest);
      }
      case "prepare_trace_enrichment": return this.store.prepareTraceEnrichment(String(required(input, "trace_id")));
      case "save_trace_enrichment": return this.store.cacheTraceEnrichment({
        traceId: String(required(input, "trace_id")),
        title: String(required(input, "title")),
        summary: String(required(input, "summary")),
        stages: required(input, "stages") as never,
        outcome: String(required(input, "outcome")) as never,
        provider: "local_subscription",
        model: input.model ? String(input.model) : undefined,
        promptVersion: String(required(input, "prompt_version")),
      });
