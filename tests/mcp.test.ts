...
test("MCP exposes ten bounded retrieval and mutation tools over JSON-RPC", () => {
  assert.equal(MCP_TOOLS.length, 10); assert.equal(new Set(MCP_TOOLS.map((tool) => tool.name)).size, 10);
  ...
  try {
    ...
    const tools = handleMcpMessage(server, { jsonrpc: "2.0", id: 2, method: "tools/list" }) as { result: { tools: unknown[] } }; assert.equal(tools.result.tools.length, 10);
